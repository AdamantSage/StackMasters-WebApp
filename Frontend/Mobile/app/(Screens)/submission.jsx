...
const Submission = () => {
  ...
  const fetchSubmissions = async (userId) => {
    try {
      ...
      const response = await axios.get(`http://192.168.61.105:5000/submissions?userId=${userId}`, {
  ...
  const fetchFeedbacks = async (subId) => {
    try {
      const token = await AsyncStorage.getItem('jwt');
      const response = await axios.get(`http://192.168.61.105:5000/feedback/${subId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
